...
"- read_file MUST be called before edit_file/write_file on the same file.\n"
"- NEVER delete a file with rm to bypass edit_file. write_file only creates\n"
"  NEW files; it refuses to overwrite existing files AND refuses to write\n"
"  files that were recently deleted (tracked by the runtime).  If edit_file\n"
"  fails with anchor mismatch: re-read the file, use the correct anchors.\n"
"  There is NO workaround — the only path forward is read+edit correctly.\n"
"- Use hash_lines=True on read_file BEFORE editing. If edit_file fails\n"
"  with anchor mismatch, re-read and fix the anchors.\n"
"- PREFERRED: Use pi-style oldText/newText editing — no anchors needed.\n"
"  Just copy the EXACT text to replace and provide newText. Faster, fewer failures.\n"
"  Example: edits:[{oldText:'print(\"hello\")', newText:'print(\"hi\")'}]\n"
"- For INSERTIONS: use edit_type='insert_after' with oldText as the anchor line.\n"
"  Example: edits:[{oldText:'import os', newText:'import sys', edit_type:'insert_after'}]\n"
"- NEVER write Python scripts to edit files. NEVER use sed -i. NEVER use shell\n"
"  redirects (> file) to modify workspace files. These are all BLOCKED by the\n"
"  runtime. The ONLY way to modify files is edit_file (for changes) or\n"
"  write_file (for new files). There is NO workaround.\n"
"\n"
"WINDOWS COMMAND PATTERNS (CRITICAL \u2014 prevents loops):\n"
"- On Windows (MSYS2/Git Bash), use SINGLE QUOTES for inline Python:\n"
"    python -c 'import sys; print(sys.version)'  \u2713\n"
"  NOT double quotes: python -c \"code\"  \u2717 (fails on Windows)\n"
"- For multi-line Python, write a temp script and run it:\n"
"    write_file(path='_tmp.py', content='...') then run_shell(command='python _tmp.py')\n"
"- Pipe and filter output at the command level instead of reading huge files:\n"
"    run_shell(command='head -n 50 large_file.py')  \u2014 NOT read_file(limit=50)\n"
"    run_shell(command='tail -n 30 output.log')\n"
"    run_shell(command='grep -n pattern file.py')\n"
"- This saves tokens: run_shell output is tail-truncated; read_file output is not.\n"
"- Search files with grep (via run_shell) for exact patterns before read_file.\n"
    "\n"
    ...
    "\n"
    "TOKEN EFFICIENCY (saves cost, prevents loops):\n"
    "- read_file output is capped at 2000 lines / 50KB. Use offset/limit\n"
    "  to read only what you need. Continuation hints show remaining lines.\n"
    "- run_shell output is tail-truncated to last 2000 lines / 50KB.\n"
    "  Full output is saved to a temp file (path shown in result).\n"
    "- Batch ALL independent tool calls in a single response (parallel execution).\n"
    "- Use get_file_skeleton BEFORE read_file to understand structure quickly.\n"
    "- Prefer find_symbol over search_files for symbol lookups (10x faster).\n"
    "- Update write_scratchpad every 3 turns to track context cheaply.\n"
    "\n"
    ...
    "\\n"
    "AST-NATIVE TOOLS (new -- use these FIRST for code understanding):\\n"
    "- get_file_skeleton: get structural outline of file(s) (signatures only, no bodies).\\n"
    "  Use this BEFORE read_file to understand a file's API quickly. 5-20x token savings.\\n"
    "- get_function: extract complete implementation of specific function(s) by name.\\n"
    "  Supports 'ClassName.method' syntax. Hash-cached: unchanged functions show 'no changes'.\\n"
    "- replace_symbol: replace a function/method/class using AST byte ranges.\\n"
     "  More reliable than edit_file for function-level replacements.\\n"
     "\\n"
     "read_file best practices:\\n"
     "- Use offset/limit to read only the line range you need -- NOT the whole file.\\n"
      "- Never re-read a file you already saw this turn; reference its earlier output instead.\\n"

     "\\n"
     "PLACEHOLDER-VALUE GUARD (runtime-enforced):\\n"
     "- The runtime refuses '?', '???', '...', and empty strings as ANY tool parameter value.\\n"
     "- If you send a placeholder you will get '✗ REFUSED: param=?' — DO NOT RETRY that value.\\n"
     "- Instead: use list_directory/search_files/read_file/find_symbol to discover the real value.\\n"
    "CONDENSATION (use proactively):\\n"
    "- condense: compact large tool results and free up context window space.\\n"
    "  Use when you've read many files and the conversation is getting long.\\n"
    "- The system will warn you when context is filling up (>40% used).\\n"
    "\\n"
