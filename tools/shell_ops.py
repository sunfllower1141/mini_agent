...
_DANGEROUS_COMMANDS: list[tuple[str, str]] = [
    ...
    # Block script-based file editing -- use edit_file instead
    (r"\bsed\s+.*-i", "sed -i: in-place file edit -- use edit_file with oldText/newText instead"),
    (r"\bopen\(.*['\"].*\bw['\"]", "open(file,'w'): writing files via Python -- use edit_file or write_file instead"),
    (r">\s*\S+\.py", "redirect to .py file: overwriting via shell -- use edit_file instead"),
    (r">\s*\S+\.txt", "redirect to .txt file: overwriting via shell -- use write_file instead"),
    (r">\s*\S+\.md", "redirect to .md file: overwriting via shell -- use write_file instead"),
    (r">\s*\S+\.js", "redirect to .js file: overwriting via shell -- use edit_file instead"),
    (r">\s*\S+\.ts", "redirect to .ts file: overwriting via shell -- use edit_file instead"),
