...
def main() -> None:
    ...
    try:
        while True:
            ...
            if user_input == "/init":
                print("  Analyzing codebase to generate .mini_agent.rules...")
                try:
                    import glob
                    import time
                    rules_path = os.path.join(workspace, ".mini_agent.rules")
                    py_files = sorted(glob.glob(os.path.join(workspace, "*.py")))
                    rules = [
                        ...
                        "",
                        "## Testing",
                        f"- Run tests with: python -m pytest -q",
                        "",
                        "## Module Map",
                    ]
                    for pf in py_files[:25]:
                        name = os.path.basename(pf)
                        rules.append(f"  {name}  # auto-detected")
                    with open(rules_path, "w") as f:
                        f.write("\n".join(rules))
                    print(f"  Created {rules_path} ({len(rules)} lines)")
